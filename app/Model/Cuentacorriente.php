 ...
 */
	...
	function AltaCuenta($data = null){
		if(!empty($data)){
			...
			$this->create();
			$nrocuentacorriente = str_pad($data['Cuenta']['cliente_id'],4,'0',STR_PAD_RIGHT)+str_pad($data['Cuenta']['tallercito_id'],4,'0',STR_PAD_LEFT);
			...
			if($this->save($data)){
				...
				return 'No se pudo recuperar el numerador para Cuenta Corriente';
			}else{
				$datasource->rollback($this);
