...
function seleccionarcliente(fila){
	...
	var ClienteTelefono = $('#ClienteTelefono'+fila).val()
	var ClienteDomicilio = $('#ClienteDomicilio'+fila).val()
	if(typeof(ClienteId) != 'undefined' && typeof(ClienteDocumento) != 'undefined'){
		...
		if(typeof(clienmodalnameset)!='undefined'){
			...
			$('#BicicletareparamoTelefono').val(ClienteTelefono)
			$('#BicicletareparamoDireccion').val(ClienteDomicilio)
			reloadList()
		}
