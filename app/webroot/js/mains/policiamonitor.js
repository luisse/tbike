...
var config = {
  ...
  authDomain: "tst-taxiar.firebaseapp.com",
...
function IniciarEventos(){
	...
	firebase.database().ref('firebase/panic').on('value', function(data) {
		refreshNow();
	});
...
function decrementTimer() {
	timeLeft--;
	$('#time_left').html('<span>Actualizando información en ' +timeLeft+'</span>');
	if (timeLeft<=0) {
		get_kpi();
    get_panic();
...
function showDetails(){
		if (kpi_selected == 1) {
			view_kpi('kpi_ocupado_list','success','Ocupado');
			return;
		}

		if (kpi_selected == 2) {
			view_panic('kpi_ocupado_list','danger','Alarma Activada');
			return;
		}
}

function show_live_panic(){
    get_panic();
    setInterval(show_live_panic,10000);
}

function view_panic(data,css_class,kpi_name) {
	$('#view_detail_kpi').html('<thead> <tr> <th>Apellido y Nombre</th><th>Teléfono</th> <th>Patente</th> <th>Licencia</th> </tr>');
	$('#detail_kpi').html('<h3><span class="label label-'+css_class+'">'+kpi_name+'</span></h3>');
	var items = [];
	  $.each( panic_json.panics, function( key, val ) {
	  		var row = $("<tr />");
			 $("#view_detail_kpi").append(row);
   			...
   			row.append($("<td>" + val.licencenumber + "</td>"));
				MapsShow(val.lat,val.lng,val.carcode,val.licencenumber);
	});
	viewmapscenter();
  //live panic
  //show_live_panic();
...
function view_kpi (data,css_class,kpi_name) {
	$('#view_detail_kpi').html('<thead> <tr> <th>Apellido y Nombre</th> <th>Patente</th> <th>Licencia</th> <th>Ver en Mapa</th> </tr>');
	$('#detail_kpi').html('<h3><span class="label label-'+css_class+'">'+kpi_name+'</span></h3>');
...
function get_kpi(){
	var is_test = false;
	var myUrl = '/kpis/kpis_count.json?is_test='+is_test;
	$('#alert').hide(1);
	$('#pingpong').html("<span class='label label-alert'>Enviando</span>");
	$.ajax({url:myUrl,
		type:'get',
		dataType:'json',
		success: function(data){
			  	if(data){
			  		$('#pingpong').html("<span class='label label-success'>Recibido</span>");
		...
		error: function () {
			$('#alert').show(1);
			$('#msg').html('&nbsp;Error con la conexion al servidor. Verifique su conexión al servidor')
		 }
	})
}
...
function get_panic(){
  //var is_test = false;
  var myUrl = '/taxpanics/getpanic.json?is_test='+test;
	setMapOnAll(null);
	$.ajax({url:myUrl,
		type:'get',
		dataType:'json',
		success: function(data){
			  	if(data){
			  		$('#panico_activado').html(data.total_panics);
						if(parseInt(data.total_panics) > 0){playSound(); kpi_selected = 2;}
			  		panic_json = data;
			  	}
			  	showDetails();
		},
